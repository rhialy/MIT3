...
router.get('/songs', function(req, res, next) {
  ...
  if (req.query.artist === undefined) {
    ...
    var songs = [];
    db.each('SELECT songs.artist as artist, songs.title as title, songs.song_length as length FROM songs, artists_songs WHERE songs.ID = artists_songs.song_id AND artists_songs.artist_id = ? ORDER BY title ASC', req.query.artist, function(err,row) {
        
        var minutes = Math.floor(row.length/60);
        var seconds = row.length % 60;
        row.length = minutes + ":" + seconds + " Minuten";
      songs.push(row);
      ...
      if (err) {
        ...
        res.render('songs', { artist: artist, songs: songs, tracks: songs.length });
      }
